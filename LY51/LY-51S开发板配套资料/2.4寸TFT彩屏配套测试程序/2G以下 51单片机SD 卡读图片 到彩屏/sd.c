/*-----------------------------------------------
  ...
  注意事项：由于SD卡使用SPI模式，并且是块读取(512Byte),所以需要单片机提供大于512的可用RAM，以供缓冲使用
            这里可以选择STC89c54RD+、STC89C58RD+、STC89C516RD+，后缀RD+表明扩展RAM是1024字节，RC表明扩展RAM256字节
------------------------------------------------*/
#include <reg52.h>
#include <stdio.h>
#include<9325TP.h>
...
main()
{
 ...
 unsigned long j;  //执行循环需要的临时变量
  unsigned int i;
  unsigned long AddTemp=393728;//SD卡地址第一个数据物理地址初始值，可以用winhex查看，这里是512扇区，512x512=262144，根据实际SD卡内容更改
  	CS=1;
	delayms(5);
	RES=0;
	delayms(5);
