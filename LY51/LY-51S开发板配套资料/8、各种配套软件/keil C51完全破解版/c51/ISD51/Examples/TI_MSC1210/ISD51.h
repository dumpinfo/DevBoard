...
// The MSC1210 has a flash page size of 128 bytes
// The Hardware breakpoints must be disabled in this case
#define CBLK_SZ   128      // Flash block size for CWRITE, valid are:
...
;        MOV    R3,#0      ; select program flash and not data flash
;        LCALL  PAGE_ERASE ; expects address in R6/R7 and R3=0 for program flash
;                          ; PAGE_ERASE leaves the address value in DPTR
;        MOV    A,#1       ; Erase Error
;        CJNE   R7,#0,CW_Error

         MOV    DPL,R0     ; address LSB
         MOV    DPH,A      ; address MSB
         MOV    FMCON,#52H ; enable page erase, delay line, 
         LCALL  WRITEFLASH ; write one dummy byte into flash page to erase the page
         MOV    FMCON,#12H ; disable page erase, delay line, 
...
REP_PRG: MOV    A,@R1      ; read next byte from buffer
         MOV    R6,A       ; save value for compare
         LCALL  WRITEFLASH ; expects address in DPTR and value in ACC
         CLR    A
