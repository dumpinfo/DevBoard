...
void LCD_MM()
{
  ...
  for(count=0;count<24;count++)		
   		{
			for(num=0;num<512;num++)			//从Flash中将数据读到RAM中用于显示
			 {		
				RamVar_1[num]=pgm_read_byte( &gImage_mmp[num+count*512] );
			 }
			p = RamVar_1;
			for(num=0;num<512;num++)	        //将读出来的数据写入TFT的GRAM中用于显示		
			 {		
...
void LCD_QQ()
{
	...
	for(count=0;count<8;count++)		
   		{
			for(num=0;num<400;num++)		//从Flash中将数据读到RAM中用于显示
			{		
				RamVar_1[num]=pgm_read_byte( &gImage_qqp[num+count*400] );

			}

			p = RamVar_1;	 
			
			for(num=0;num<400;num++)	    //将读出来的数据写入TFT的GRAM中用于显示
			{		
	...
	for(count=0;count<40;count++)		
   		{
			for(num=0;num<512;num++)	  			//从Flash中将数据读到RAM中用于显示
			{		
				RamVar_1[num]=pgm_read_byte( &gImage_ph1[num+count*512] ); //数据组1读取
			}
			p = RamVar_1;	 
	...
	for(count=0;count<40;count++)		
   		{
			for(num=0;num<512;num++)				//从Flash中将数据读到RAM中用于显示
			{		
				RamVar_1[num]=pgm_read_byte( &gImage_ph2[num+count*512] );  //数据组2读取
