...
//**************控制端口定义********************
#define DataPort PORTB     //数据口使用DataPort

#define RS  RD5		//数据/命令选择
#define RW  RD4     //写数据/命令
#define nRD RD3     //读控制
#define CS  RD2		//片选
#define RES RD1		//复位

#define LE  RD0	    //74HC573锁存控制
//**************声明外部函数和变量**************
...
void Put16x16(unsigned short x, unsigned short  y, unsigned char g[], unsigned int fColor,unsigned int bColor)
{
	unsigned int i,j,k;

	LCD_SetPos(x,  x+16-1,y, y+16-1);                       //设置汉字显示位置

	for (k=0;k<16;k++)                                      //循环64次，查询汉字字模位置
	{ 
...
//		        bColor 字符背景颜色
//================================================================================================
void LCD_PutString(unsigned int x, unsigned int y, unsigned char tab0[], unsigned int fColor, unsigned int bColor) 
{
	 unsigned char l=0,i=0,gbk[2];                            //显示屏位置增量
     
     while(tab0[i]) 
	 {
		if( tab0[i] < 0x80)                             //判断s指向的字符串中的某字符的编码值是否小于128,如果小于,即为ASCII字符
		    {
			 LCD_PutChar(x+l*8,y,tab0[i],fColor,bColor);//显示该字符
		     i++;l++;                              //指针加1,位置加1
			}
		else
		    {
             gbk[0]=tab0[i];gbk[1]=tab0[i+1];
			 Put16x16(x+l*8,y,gbk,fColor,bColor);//显示该汉字
		     i+=2;l+=2;                           //因为汉字为编码为2字节，指针加2,显示16x16所以位置加2
			}
	 }
}
