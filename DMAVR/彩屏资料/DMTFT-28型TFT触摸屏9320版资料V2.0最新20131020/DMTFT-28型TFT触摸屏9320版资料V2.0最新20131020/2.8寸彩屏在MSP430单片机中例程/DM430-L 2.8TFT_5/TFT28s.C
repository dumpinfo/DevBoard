...
void LCD_PutChar(unsigned short x, unsigned short y, char c, uint fColor, uint bColor)
{
 ...
 for(i=0; i<16;i++) {                             //循环写入16字节，一个字符为16字节
		uchar m=Font8x16[(c-0x20)*16+i];  //提取c字符的第i个字节以,c减去0x20是由于Ascii码库中的0~1f被去掉
...
输入参数：x 横坐标
	  ...
	  fColor 字符颜色
          bColor 字符背景颜色
***********************************************************************/
void PutGB2424(unsigned short x, unsigned short  y, uchar c[2], uint fColor,uint bColor){
	uint i,j,k;
...
void LCD_PutString(unsigned short x, unsigned short y, uchar *s, uint fColor, uint bColor) 
{
     ...
     while(*s) 
	 {
		if( *s < 0x80)                             //判断s指向的字符串中的某字符的编码值是否小于128,如果小于,即为ASCII字符
		    ...
		    {
			 Put16x16(x+l*8,y,(uchar*)s,fColor,bColor);//显示该汉字
...
void LCD_PutString24(unsigned short x, unsigned short y, uchar *s, uint fColor, uint bColor) {
	...
	while(*s) {
		if( *s < 0x80) 
		    ...
		    {
			PutGB2424(x+l*12,y,(uchar*)s,fColor,bColor);
			s+=2;l+=2;
			}
		}
	}
