...
#define mmcReadSector(sector, pBuffer) mmcReadBlock(sector, 512, pBuffer)  //读扇区函数，参数为扇区号与存放数据地址
uint8 mmcWriteBlock(uint32 address, uint16 count, uint8 *pBuffer);
#define mmcWriteSector(sector, pBuffer) mmcWriteBlock(sector, 512, pBuffer) //写扇区函数，参数为扇区号与需写入数据地址
